...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    ...
    /*def descendingByRetweet: TweetList = {
      def loop(in: TweetSet, ts: TweetList): TweetList = {
        println("I am here inside loop")
        if(in.isEmpty) ts
        else{
          val mostPopular = in.mostRetweeted
          println(s"most $mostPopular")
          new Cons(mostPopular, loop(in.remove(mostPopular), ts))
        }
      }
      println("trying to enter loop")
      loop(this, Nil)
    }*/
